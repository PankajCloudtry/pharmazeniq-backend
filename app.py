...
.block-container{padding:0 1rem;}              /* ← no fixed height ⇒ no cut-off */
...
def match_opts(key: str):
    return [n for n, score, _ in process.extract(key, meds_df.name, limit=5) if score >= 50]
...
with tabs[1]:
    ...
    if "raw" not in st.session_state:
        ...
        # glue keyword line + next non-keyword line
        merged: List[str] = []
        i = 0
        while i < len(raw_lines):
            line = raw_lines[i]
            lower = line.lower()
            if any(t in lower for t in TOKENS) and i + 1 < len(raw_lines):
                nxt = raw_lines[i + 1]
